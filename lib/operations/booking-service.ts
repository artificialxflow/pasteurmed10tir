...
export async function createBookingRecord(body: CreateBookingInput) {
  ...
  const appointmentDate = body.appointmentDate ? String(body.appointmentDate) : null;
  const day =
    body.day
      ? String(body.day)
      : appointmentDate
        ? persianWeekdayFromIsoDate(appointmentDate)
        : null;
  ...
  const dateLabel = appointmentDate
    ...
    : body.dateLabel || new Date().toLocaleDateString('fa-IR');
  ...
  if (isSmsConfigured()) {
    ...
    if (recent === 0) {
      const timeLabel = row.timeLabel
        ? `${row.dateLabel || row.day || ''} ${row.timeLabel}`.trim()
        : row.day
          ? `${row.day} ${row.timeValue ?? ''}`
          : '—';
