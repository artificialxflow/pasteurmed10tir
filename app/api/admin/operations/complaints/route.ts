...
import { mapComplaint } from '@/lib/operations/mappers';
...
export async function GET() {
  ...
  return NextResponse.json({ items: rows.map(mapComplaint) });
}
...
export async function PATCH(request: Request) {
  ...
  const status =
    body.status === 'new' || body.status === 'reviewing' || body.status === 'closed'
      ? body.status
      : undefined;
  ...
  return NextResponse.json({ item: mapComplaint(row) });
}
