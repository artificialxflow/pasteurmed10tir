...
import { nextInstallmentDue, remainingInstallment, type InstallmentPlan } from "@/lib/patient";
...
export function InstallmentsPage({ variant = "web" }: { variant?: "web" | "app" }) {
  ...
  return (
    <div className={variant === "app" ? "space-y-4" : "mx-auto max-w-2xl space-y-4 px-4 py-10"}>
      ...
      <p className="text-sm text-slate-600">مبلغ پرداخت‌شده، مانده و تاریخ سررسید بعدی</p>
      {plans.map((plan) => {
          ...
          <Card key={plan.id} hover={false} className="space-y-2 p-5 text-sm">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="rounded-full bg-cyan-50 px-2 py-0.5 text-xs font-bold text-cyan-800">
                {plan.source === "membership" ? "عضویت" : "کیف اعتبار"}
              </span>
            </div>
            ...
            <p>سررسید بعدی: {nextDue || "—"}</p>
          </Card>
        );
      })}
      {!plans.length ? (
        <p className="text-center text-sm text-slate-500">
          هنوز طرح اقساطی ثبت نشده است. پس از پرداخت عضویت نمونه ساخته می‌شود.
        </p>
      ) : null}
    </div>
  );
