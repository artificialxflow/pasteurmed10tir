...
export function ConfirmLaserPayment({ basePath }: { basePath: LaserBasePath }) {
  ...
  const body = (
    <>
      {!app ? (
        <>
          <h1 className="mb-2 text-center text-2xl font-bold text-slate-900">تأیید بیعانه نوبت لیزر</h1>
          <p className="mb-8 text-center text-slate-600">با پرداخت بیعانه، زمان انتخاب‌شده رزرو می‌شود</p>
        </>
      ) : null}
      ...
      <Button onClick={onPay} disabled={paying} variant="accent" className="mb-3 w-full py-3 text-base">
        {paying ? "در حال اتصال به درگاه..." : "پرداخت بیعانه و انتقال به درگاه"}
      </Button>
